...
//
// 【首页-电子台账】检查数码监控录像情况记录

import Foundation
import UIKit

class CheckVideoVC: BaseFormVC {
    let listV = CheckVideoView()
    ...
    override func didValidated() {
        ...
        do {
            ...
            valPam.tjrq = Date.dateToUp(listV.val1TF.value ?? "")
            valPam.attrValue0 = listV.val0TF.value
            valPam.attrValue1 = Date.dateToUp(listV.val1TF.value ?? "")
            valPam.attrValue2 = listV.val2TF.value
            valPam.attrValue3 = Date.dateToUp(listV.val3TF.value ?? "")
